...
export default function LoginPage() {
  ...
  //  Canonical site URL (NEVER allow 0.0.0.0 to leak into OAuth)
  // - Prefer NEXT_PUBLIC_SITE_URL if valid and not 0.0.0.0
  // - Else fallback to window.origin, but sanitize 0.0.0.0 -> localhost
  ...
  const redirectTo = useMemo(() => {
    ...
    //  preserve next through callback
  ...
  const safeSetMessage = useCallback((m: { type: 'success' | 'error'; text: string } | null, ms?: number) => {
      ...
      <div className="bg-card p-8 rounded-2xl shadow-lg w-full max-w-md border border-border space-y-6">
        ...
        {tab === 'google' && (
          <div id={panelId('google')} role="tabpanel" aria-labelledby={tabId('google')} className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">You’ll be redirected to Google to choose an account.</p>
          </div>
        )}
